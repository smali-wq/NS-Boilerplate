                    ...
                    <li key={step.name} className="md:flex-1">
                        {step.status === 'complete' ? (
                            <div className="group flex flex-col border-l-4 border-blue-600 py-2 pl-4 transition-colors md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4">
                                <span className="text-[10px] font-black uppercase tracking-widest text-blue-600">Step {idx + 1}</span>
                                <span className="text-sm font-black uppercase text-slate-900">{step.name}</span>
                            </div>
                        ) : step.status === 'current' ? (
